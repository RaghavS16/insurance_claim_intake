...
plus a short, grounded conversational reply. No raw utterance is stored as a
claim field without normalization.
"""
...
Use the conversation history only to understand references such as 'the amount', 'that date', or 'the same policy'; do not re-ask for facts already present.
For event_description, return only the incident narrative; exclude policy numbers, dates, locations, amounts, greetings, and assistant text.
...
For spoken_reply, write ONE short natural sentence or question (roughly 6-25 words) that is grounded only in the current state and latest utterance. Do not summarize the entire claim on every turn. Do not invent facts. If a required field is missing, ask naturally for the most useful missing field. If the claimant supplied several facts, acknowledge them briefly and ask only for the next missing fact. If all required fields are present, ask for confirmation rather than submitting anything.
"""
...
def _deterministic_date(text: str, reference: date) -> Optional[str]:
    ...
    patterns = [r"\b(\d{4}-\d{1,2}-\d{1,2})\b", r"\b(\d{1,2}[/-]\d{1,2}[/-]\d{4})\b", r"\b(\d{1,2}\s+(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s+\d{4})\b", r"\b((?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s+\d{1,2}\s+\d{4})\b"]
...
def _parse_indian_amount(text: str) -> Optional[float]:
    ...
    return value * {"k": 1_000, "thousand": 1_000, "lakh": 100_000, "lakhs": 100_000, "lac": 100_000, "lacs": 100_000, "crore": 10_000_000, "crores": 10_000_000, "cr": 10_000_000}.get(unit, 1)
...
def _deterministic_amount(text: str) -> Optional[float]:
    low = text.lower()
    patterns = [r"(?:₹|rs\.?|inr|rupees?|repair\s+cost|repair|damage|loss|estimated\s+(?:cost|loss)|claim|cost|bill|worth)[^\d]{0,25}(\d[\d,]*(?:\.\d+)?)\s*(crores?|cr|lakhs?|lacs?|lac|k|thousand)?\b", r"(\d[\d,]*(?:\.\d+)?)\s*(crores?|cr|lakhs?|lacs?|lac|k|thousand)\b"]
    for pattern in patterns:
...
def _deterministic_policy(text: str) -> Optional[str]:
    patterns = [r"\bpolicy\s*(?:number|no\.?|id|identifier)\s*(?:is|:)?\s*([A-Za-z0-9][A-Za-z0-9_-]{2,})", r"\bpolicy\s*(?:is|:)\s*([A-Za-z0-9][A-Za-z0-9_-]{2,})"]
    for pattern in patterns:
...
def _deterministic_location(text: str) -> Optional[str]:
    ...
    for match in re.finditer(pattern, text, re.I):
        ...
        if candidate and candidate.lower() not in {"the road", "there"}: return candidate.title()
    return None
...
def _strip_incident_noise(text: str) -> str:
    ...
    value = re.sub(r"\s+([,.!?])", r"\1", value); value = re.sub(r"\s{2,}", " ", value).strip(" ,.-")
    return value
...
def _normalize_description(value: Any, raw: str) -> Optional[str]:
    ...
    metadata_hits = sum(bool(re.search(pattern, candidate, re.I)) for pattern in (r"\bpolicy\b", r"\b(?:rupees?|rs\.?|inr|₹)\b", r"\b(?:yesterday|today|tomorrow)\b") )
    if metadata_hits >= 2: candidate = _strip_incident_noise(candidate)
...
def _rule_changes(raw: str, state: ClaimState) -> List[FieldChange]:
    ...
    if d and not current.get("event_date"): changes.append(FieldChange(field="event_date", operation="set", value=d, evidence=raw, confidence=0.99))
    ...
    if itype and not current.get("insurance_type"): changes.append(FieldChange(field="insurance_type", operation="set", value=itype, evidence=raw, confidence=0.95))
    amount = _deterministic_amount(raw)
    if amount is not None and current.get("estimated_claim_amount") in (None, "", UNKNOWN_SENTINEL): changes.append(FieldChange(field="estimated_claim_amount", operation="set", value=amount, evidence=raw, confidence=0.99))
    location = _deterministic_location(raw)
    if location and not current.get("event_location"): changes.append(FieldChange(field="event_location", operation="set", value=location, evidence=raw, confidence=0.92))
    return changes


def _merge_change(state: ClaimState, change: FieldChange, turn: int) -> bool:
    data = state.setdefault("extracted_data", {}); metadata = state.setdefault("field_metadata", {}); statuses = state.setdefault("field_status", {})
    ...
    data[change.field] = value; statuses[change.field] = "corrected" if change.operation == "replace" and old not in (None, "", UNKNOWN_SENTINEL) else "provided"
...
def _validate_change(change: FieldChange, state: ClaimState) -> Optional[FieldChange]:
    ...
    if change.confidence < 0.55: return None
...
def conversation_turn_processor(state: ClaimState) -> ClaimState:
    raw = str(state.get("claim_text") or "").strip(); state["last_user_utterance"] = raw
...
Missing required fields: {json.dumps(state.get('missing_fields', []), ensure_ascii=False)}
Recent conversation:
{_history_text(state)}
...
"""
    ...
    if patch is None: patch = ExtractionPatch(intent=_fallback_intent(raw, awaiting), changes=[])
    state["last_intent"] = patch.intent; state["spoken_response"] = patch.spoken_reply.strip() if patch.spoken_reply else ""

    deterministic = _rule_changes(raw, state); deterministic_fields = {c.field for c in deterministic}; model_changes = [c for c in patch.changes if c.field not in deterministic_fields]
    ...
    if patch.intent == "escalation":
        state["escalate_to_human"] = True; state["escalation_reason"] = "user_requested"; state["conversation_status"] = "escalated"; state["_skip_all"] = True; state["next_question"] = "I understand. I'll connect you with a claims specialist who can help you directly."; state["message"] = state["next_question"]; return state
    if patch.intent == "closing":
        state["_skip_all"] = True; state["next_question"] = "Of course. We can continue whenever you're ready."; state["message"] = state["next_question"]; return state
    if patch.intent == "defer":
        target = state.get("next_question_field")
        if target:
            unknown = state.setdefault("unknown_fields", [])
            if target not in unknown: unknown.append(target)
...
def mandatory_field_checker(state: ClaimState) -> ClaimState:
    ...
    data = state.get("extracted_data", {}); statuses = dict(state.get("field_status", {})); metadata = state.setdefault("field_metadata", {}); missing: List[str] = []
...
def _confirmation_summary(data: Dict[str, Any]) -> str:
    parts = []
    if data.get("insurance_type"): parts.append(SUPPORTED_INSURANCE_TYPES.get(data["insurance_type"], str(data["insurance_type"])))
    if data.get("event_date"): parts.append(data["event_date"])
    if data.get("event_location"): parts.append(f"in {data['event_location']}")
    if data.get("estimated_claim_amount") is not None: parts.append(f"with estimated damage of ₹{float(data['estimated_claim_amount']):,.0f}")
    if data.get("policy_id"): parts.append(f"under policy {data['policy_id']}")
    description = data.get("event_description")
    sentence = "I have your claim details: " + (description.rstrip('.') + ". " if description else "")
    return sentence + ("Please confirm that everything is correct." if parts else "Is everything correct?")
...
def _deterministic_response(state: ClaimState) -> str:
    ...
    missing = state.get("missing_fields", []); target = state.get("next_question_field") or (missing[0] if missing else None)
...
def next_question_generator(state: ClaimState) -> ClaimState:
    if state.get("_skip_all"): return state
    if state.get("awaiting_confirmation"): state["next_question_field"] = "confirmation"
    else:
        missing = state.get("missing_fields", []); hinted = state.get("current_field_hint"); state["next_question_field"] = hinted if hinted in missing else (missing[0] if missing else "confirmation")
    deterministic = _deterministic_response(state)
    semantic = str(state.get("spoken_response") or "").strip()
    # Never allow the model to replace a mandatory confirmation prompt or a clearly unsafe response.
    if state.get("awaiting_confirmation"):
        state["next_question"] = deterministic
    elif semantic and 6 <= len(semantic.split()) <= 35:
        state["next_question"] = semantic
    else:
        state["next_question"] = deterministic
