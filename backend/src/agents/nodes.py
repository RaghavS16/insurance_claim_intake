...
Event description means only the actual incident narrative. Exclude policy numbers, dates, locations, amounts, greetings, and assistant text.
Social turns and process questions must not modify claim data. Corrections replace only the corrected field. Additional incident details may append to an existing narrative.
Confirmation/rejection creates no new facts unless the same utterance explicitly contains a correction. A policy identifier normally contains digits. Never turn generic words into a policy value.
...
def _clean_policy_candidate(value: Any) -> Optional[str]:
    ...
    raw = re.sub(r"\s+", "-", raw); clean = re.sub(r"[^A-Za-z0-9_-]", "", raw).upper()
...
def _rule_changes(raw: str, state: ClaimState) -> List[FieldChange]:
    ...
    if amount is not None:
        old_amount = current.get("estimated_claim_amount"); replace_amount = old_amount not in (None, "", UNKNOWN_SENTINEL) and bool(re.search(r"\b(?:actually|correction|correct|instead|change|make it|update|revised?)\b", low))
    ...
    if location:
        old_location = current.get("event_location"); replace_location = old_location not in (None, "", UNKNOWN_SENTINEL) and bool(re.search(r"\b(?:actually|instead|correct|change|update|revised?)\b", low))
...
def conversation_turn_processor(state: ClaimState) -> ClaimState:
    ...
    # Use deterministic extraction first. This is the normal path for baseline
    # claim facts and keeps CI/test environments independent of Ollama.
    deterministic = _rule_changes(raw, state)
    deterministic_fields = {c.field for c in deterministic}
    patch: Optional[ExtractionPatch] = None
    if not deterministic:
        ...
        patch = ExtractionPatch(intent="correction" if re.search(r"\b(?:actually|correct|change|update|instead|revised?)\b", low) else "claim_detail", changes=[])
    ...
    if not state["recently_extracted_fields"] and state["last_intent"] in {"claim_detail", "unclear", "filler"} and isinstance(target_before, str):
        ...
        if isinstance(target_before, str): retries[target_before] = 0
...
def _confirmation_summary(data: Dict[str, Any]) -> str:
    label = SUPPORTED_INSURANCE_TYPES.get(data.get("insurance_type"), str(data.get("insurance_type"))) if data.get("insurance_type") else ""; date_text = str(data.get("event_date") or ""); location = str(data.get("event_location") or ""); amount = data.get("estimated_claim_amount"); policy = data.get("policy_id"); description = str(data.get("event_description") or "").strip().rstrip("."); lead = f"I have your {label.lower() if label else 'claim'}"; details = []
