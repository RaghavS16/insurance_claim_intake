...
if _IS_PG:
    ...
    def _UUID(*args, **kw): return mapped_column(PgUUID(as_uuid=True), *args, **kw)
    def _JSONB(*args, **kw): return mapped_column(PgJSONB, *args, **kw)
else:
...
class Claim(Base):
    ...
    insurance_type: Mapped[Optional[str]] = mapped_column(String, index=True)
    input_mode: Mapped[Optional[str]] = mapped_column(String, default="text")
