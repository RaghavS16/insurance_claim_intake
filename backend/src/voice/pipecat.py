...
class WebRTCVADAnalyzer(VADAnalyzer):
    def __init__(self, aggressiveness: int = 1, *, sample_rate: int = 16000):
        super().__init__(sample_rate=sample_rate, params=VADParams(confidence=0.5, start_secs=0.08, stop_secs=0.35, min_volume=0.0))
...
class ClaimAgentProcessor(FrameProcessor):
    ...
    async def process_frame(self,frame:Frame,direction:FrameDirection):
        ...
        if isinstance(frame,InterruptionFrame):
            self._generation+=1
            self._pending_text=""
            await self.push_frame(OutputTransportMessageFrame(message={"type":"barge_in","generation":self._generation}),direction)
            await self.push_frame(frame,direction); return
        if isinstance(frame,InterimTranscriptionFrame):
            self._segment_number+=1
            await self.push_frame(OutputTransportMessageFrame(message={"type":"transcript","speaker":"claimant","text":frame.text,"is_final":False,"segment_id":f"claimant-live-{self._segment_number}"}),direction); await self.push_frame(frame,direction); return
    ...
    async def _flush_after_pause(self,direction):
        try:
    ...
    async def _turn_worker(self):
        try:
            while True:
                ...
                try: await self._handle_final_transcript(text,direction,generation)
                except asyncio.CancelledError: raise
                except Exception: logger.exception("Queued voice turn failed")
                finally: self._turn_queue.task_done()
        except asyncio.CancelledError: return
    async def _handle_final_transcript(self,text,direction,generation):
        segment_id=f"claimant-{self._segment_number or 1}"
        await self.push_frame(OutputTransportMessageFrame(message={"type":"transcript","speaker":"claimant","text":text,"is_final":True,"segment_id":segment_id,"generation":generation}),direction)
