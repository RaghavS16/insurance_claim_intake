...
class WebRTCVADAnalyzer(VADAnalyzer):
    def __init__(self, aggressiveness: int = 1, *, sample_rate: int = 16000):
        super().__init__(sample_rate=sample_rate, params=VADParams(confidence=0.5, start_secs=0.08, stop_secs=0.3, min_volume=0.0))
        ...
        try:
            ...
            async with self._session.post(self._base_url, json=payload) as response:
                if response.status != 200: yield ErrorFrame(error=f"Piper HTTP server returned {response.status}")
                    ...
                    with wave.open(io.BytesIO(wav_bytes), "rb") as wav_file: audio, sample_rate, channels = wav_file.readframes(wav_file.getnframes()), wav_file.getframerate(), wav_file.getnchannels()
                    yield TTSAudioRawFrame(audio=audio, sample_rate=sample_rate, num_channels=channels, context_id=context_id)
...
class ClaimAgentProcessor(FrameProcessor):
    ...
    def __init__(self, claim: Claim, *, input_mode: str = "voice"):
        super().__init__(); self._db = SessionLocal(); self._ticket_id = claim.ticket_id; self._input_mode = input_mode; self._segment_number = 0
    async def cleanup(self): self._db.close(); await super().cleanup()
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TranscriptionFrame) and frame.text.strip(): await self._handle_final_transcript(frame.text.strip(), direction); return
    ...
    async def _handle_final_transcript(self, text: str, direction: FrameDirection):
        ...
        if agent_text:
            await self.push_frame(OutputTransportMessageFrame(message={"type":"transcript","speaker":"agent","text":agent_text,"is_final":True,"segment_id":f"agent-{self._segment_number}"}), direction); await self.push_frame(TTSSpeakFrame(text=agent_text), direction)
