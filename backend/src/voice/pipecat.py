...
class WebRTCVADAnalyzer(VADAnalyzer):
    def __init__(self, aggressiveness: int = 1, *, sample_rate: int = 16000):
        super().__init__(
            ...
            params=VADParams(confidence=0.5, start_secs=0.08, stop_secs=0.45, min_volume=0.0),
        )
...
def _find_piper_model_path(configured_path: str | None) -> str | None:
    if not configured_path:
        return None
    candidates = [
        Path(configured_path), Path("backend") / configured_path,
        Path.cwd() / configured_path, Path.cwd() / "backend" / configured_path,
                ...
                for chunk in self._voice.synthesize(text):
                    ...
                    if audio:
                        loop.call_soon_threadsafe(queue.put_nowait, (
                            bytes(audio), int(getattr(chunk, "sample_rate", 22050)),
                            int(getattr(chunk, "sample_channels", 1)),
                        ))
            except Exception as exc:
...
class ClaimAgentProcessor(FrameProcessor):
    """Bridge finalized Pipecat utterances to the persistent claim domain."""
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TranscriptionFrame) and frame.text.strip():
            # Serialize final turns so continuous speech cannot create overlapping LLM/database turns.
    ...
    async def _handle_final_transcript(self, text: str, direction: FrameDirection):
        ...
        try:
            ...
            await self.push_frame(OutputTransportMessageFrame(message={"type": "error", "message": "I couldn't process that turn. Please try again."}), direction)
            return
        await self.push_frame(OutputTransportMessageFrame(message={
            "type": "state_update",
        ...
        if agent_text:
            await self.push_frame(OutputTransportMessageFrame(message={
                ...
                "is_final": True, "segment_id": f"agent-{self._segment_number}",
            }), direction)
...
def websocket_transport(websocket) -> FastAPIWebsocketTransport:
    return FastAPIWebsocketTransport(
        ...
        FastAPIWebsocketParams(
            ...
            serializer=PCM16WebSocketSerializer(), add_wav_header=False,
        ),
    )
