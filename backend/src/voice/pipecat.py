...
class PCM16WebSocketSerializer(FrameSerializer):
    """Browser-compatible PCM transport; each audio frame is a self-contained WAV."""
...
class PiperNativeTTSService(TTSService):
    """Native Piper adapter that streams generated chunks instead of buffering a whole utterance."""
    ...
    async def run_tts(self, text: str, context_id: str):
            ...
            try:
                for chunk in self._voice.synthesize(text):
                    ...
                    if audio:
                        item = (
                            bytes(audio),
                            int(getattr(chunk, "sample_rate", 22050)),
        ...
        thread = threading.Thread(target=synthesize, name="piper-synthesis", daemon=True)
        thread.start()
        try:
            while True:
                ...
                if isinstance(item, Exception):
                    logger.exception("Native Piper synthesis failed", exc_info=item)
...
class ClaimAgentProcessor(FrameProcessor):
    """Bridge Pipecat transcription/interruption frames to the claim domain."""
...
def build_voice_pipeline(
    ...
    *,
    ...
    if resolved_model:
        ...
        tts = PiperHTTPService(base_url=piper_url or settings.PIPER_HTTP_URL or "http://localhost:5000/synthesize", voice=piper_voice or settings.PIPER_VOICE)
    else:
