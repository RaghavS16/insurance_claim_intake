...
def test_start_voice_session(client):
    response = client.post("/api/v1/claims/voice-session", headers={"X-User-ID": "TEST_USER_ID"})
    assert response.status_code == 200
    assert response.json()["ticket_id"].startswith("CLAIM-")


def test_intake_requires_only_missing_common_fields(client):
    response = client.post("/api/v1/claims/intake", headers={"X-User-ID": "TEST_USER_ID"}, json={
...
def test_verify_is_blocked_before_confirmation(client):
    intake = client.post("/api/v1/claims/intake", headers={"X-User-ID": "TEST_USER_ID"}, json={
        "claim_text": "I had a bike accident yesterday in Bengaluru. Policy XYZ123. Repair cost is ₹20,000.",
        "input_mode": "text",
    }).json()
    response = client.post(f"/api/v1/claims/{intake['ticket_id']}/verify", headers={"X-User-ID": "TEST_USER_ID"})
...
def test_claim_confirmation_then_verification(client):
    first = client.post("/api/v1/claims/intake", headers={"X-User-ID": "TEST_USER_ID"}, json={
        "claim_text": "I had a bike accident yesterday in Bengaluru. Policy XYZ123. Repair cost is ₹20,000.",
        "input_mode": "text",
    }).json()
    tid = first["ticket_id"]
    second = client.post(f"/api/v1/claims/{tid}/text-turn", headers={"X-User-ID": "TEST_USER_ID"}, json={"text": "Yes, everything looks correct."}).json()
    assert second["confirmed"] is True
    assert second["conversation_status"] == "pending_verification"
    verify = client.post(f"/api/v1/claims/{tid}/verify", headers={"X-User-ID": "TEST_USER_ID"})
...
def test_failed_policy_verification_does_not_verify(client):
    ...
    tid = first["ticket_id"]
    client.post(f"/api/v1/claims/{tid}/text-turn", headers={"X-User-ID": "TEST_USER_ID"}, json={"text": "Yes, everything looks correct."})
    response = client.post(f"/api/v1/claims/{tid}/verify", headers={"X-User-ID": "TEST_USER_ID"})
    assert response.status_code == 200
    assert response.json()["status"] == "verification_failed"


def test_conversation_history_persists_turns(client):
    session = client.post("/api/v1/claims/voice-session", headers={"X-User-ID": "TEST_USER_ID"}).json()
    tid = session["ticket_id"]
    client.post(f"/api/v1/claims/{tid}/text-turn", headers={"X-User-ID": "TEST_USER_ID"}, json={"text": "I had a bike accident yesterday."})
    history = client.get(f"/api/v1/claims/{tid}/conversation", headers={"X-User-ID": "TEST_USER_ID"}).json()
