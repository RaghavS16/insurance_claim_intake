...
def test_wrong_policy_owner(db, client):
    # Unit check
    res = verify_policy_for_claim("XYZ123", "2025-05-05", "wrong-claimant-id", db)
    assert not res["valid"]
    assert res["reason"] == "ownership_mismatch"

    # API check: Claimant B tries to verify with Claimant A's policy
    # Default test client has user ID TEST_USER_ID, XYZ123 belongs to TEST_USER_ID.
    # If we pass header for a different user, verification fails with ownership_mismatch
...
def test_verify_policy_ownership_mismatch(db):
    res = verify_policy_for_claim("XYZ123", "2024-05-05", "wrong_user_id", db)
    assert not res["valid"]
    assert res["reason"] == "ownership_mismatch"
