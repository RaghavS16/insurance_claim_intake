...
export interface LinkedPolicySimple {
...
interface CollectedDetailsPanelProps {
  ...
  linkedPolicies?: LinkedPolicySimple[];
  ...
  return (
    <div className="w-full lg:w-[400px] xl:w-[460px] bg-[#f8fafc] flex flex-col h-full border-t lg:border-t-0 lg:border-l border-[#e2e8f0] shrink-0 overflow-hidden">
      ...
      <div className="p-4 md:p-5 border-b border-[#e2e8f0] bg-white sticky top-0 z-10 flex justify-between items-center shadow-xs shrink-0">
        <div>
          <h2 className="font-headline text-base md:text-lg font-bold text-[#0f172a]">
            Collected Details
          </h2>
      ...
      <div className="p-4 md:p-5 space-y-3.5 flex-1 overflow-y-auto">
        ...
        >
          {extractedData.policy_id && (
            <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#00647c]"></div>
          )}
          <div className="flex justify-between items-start pl-1">
            <span className="font-label text-[11px] text-[#64748b] font-bold uppercase tracking-wider">
              Policy ID
            </span>
            <button
          ...
          <div className="flex items-center gap-2 pl-1">
            ...
            <span className="font-body text-sm text-[#0f172a] font-semibold font-mono">
              {extractedData.policy_id || (
                <span className="text-[#94a3b8] font-normal italic">Not specified yet</span>
              )}
            </span>
          </div>
          ...
          {!extractedData.policy_id && (
            <div className="mt-1 pt-1 border-t border-slate-100 pl-1 space-y-2">
              {linkedPolicies.length > 0 ? (
                <div>
                  <button
                    onClick={() => setShowPolicyPicker(!showPolicyPicker)}
                    className="text-[11px] text-[#00647c] font-semibold hover:underline flex items-center gap-1 cursor-pointer"
                  >
                    <span>Choose from {linkedPolicies.length} linked policies</span>
                    <span className="material-symbols-outlined text-xs">
                      {showPolicyPicker ? "expand_less" : "expand_more"}
                    </span>
                  </button>
                    ...
                    <div className="mt-2 space-y-1.5 p-2 bg-slate-50 border border-slate-200 rounded-lg">
                      {linkedPolicies.map((p) => (
                        <div
                          key={p.policy_number}
                          onClick={() => {
                            onSelectPolicy?.(p.policy_number);
                            setShowPolicyPicker(false);
                          }}
                          className="p-2 bg-white hover:bg-cyan-50 border border-slate-200 hover:border-[#00647c] rounded-md cursor-pointer transition-colors flex items-center justify-between"
                        >
                          <div>
                            <p className="font-mono text-xs font-bold text-[#00647c]">
                              {p.policy_number}
                            </p>
                            <p className="text-[10px] text-slate-500 capitalize">
                              {p.policy_type} Insurance
                            </p>
                          </div>
                          <span className="material-symbols-outlined text-sm text-[#00647c]">
                            check_circle
                          </span>
                        </div>
                      ))}
                ...
                <Link
                  href="/link-policy"
                  className="text-[11px] text-[#00647c] font-semibold hover:underline flex items-center gap-1"
                >
                  <span>Link a policy to your account</span>
        ...
        <div className="bg-white border border-[#cbd5e1] rounded-xl p-4 flex flex-col gap-2 shadow-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-body text-sm text-[#0f172a] font-semibold capitalize">
              {extractedData.insurance_type ? (
                (SUPPORTED_INSURANCE_TYPES as any)[extractedData.insurance_type] ||
                extractedData.insurance_type
              ) : (
        ...
        <div className="bg-white border border-[#cbd5e1] rounded-xl p-4 flex flex-col gap-2 shadow-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-body text-sm text-[#0f172a] font-semibold">
              {extractedData.event_date || (
                <span className="text-[#94a3b8] font-normal italic">Waiting for date...</span>
              )}
            </span>
          </div>
        </div>

        {/* Detail Card: Incident Location */}
        <div className="bg-white border border-[#cbd5e1] rounded-xl p-4 flex flex-col gap-2 shadow-xs">
          <div className="flex justify-between items-start">
            <span className="font-label text-[11px] text-[#64748b] font-bold uppercase tracking-wider">
              Incident Location
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-body text-sm text-[#0f172a] font-semibold">
              {extractedData.event_location || (
                <span className="text-[#94a3b8] font-normal italic">Collecting location...</span>
              )}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#cbd5e1] rounded-xl p-4 flex flex-col gap-2 shadow-xs">
          ...
          <p className="font-body text-xs text-[#334155] leading-relaxed line-clamp-3">
            {extractedData.event_description || (
              <span className="text-[#94a3b8] italic">No description recorded yet</span>
            )}
          </p>
        </div>
      </div>
      ...
      <div className="p-4 md:p-5 border-t border-[#e2e8f0] bg-white shrink-0">
        <button
          ...
          className={`w-full py-3 px-4 rounded-xl font-label text-xs font-bold tracking-wider uppercase transition-all duration-200 flex items-center justify-center gap-2 shadow-xs ${
            confirmed
              ? "bg-emerald-600 text-white cursor-default"
              : "bg-[#00647c] hover:bg-[#004e61] text-white cursor-pointer active:scale-[0.99]"
          } ${submittingClaim ? "opacity-75 cursor-wait" : ""}`}
        >
          <span className="material-symbols-outlined text-base">
            {confirmed ? "check_circle" : "send"}
          </span>
          <span>
            {confirmed
              ? "Claim Submitted"
              ...
              ? "Submitting Claim..."
              : pendingCount > 0
              ? `Submit Claim (${pendingCount} Items Incomplete)`
