...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (role === "ADJUSTER" && !adjusterCode.trim()) {
      setError("Adjuster registration code is required.");
      return;
    }
    ...
    if (password.length < 8 || !/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~`])/.test(password)) {
      setError("Password must contain 8+ characters, uppercase, lowercase, digit, and special character.");
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/v1/auth/signup`, {
        ...
        body: JSON.stringify({
          ...
          role,
          adjuster_code: role === "ADJUSTER" ? adjusterCode.trim() : null,
        }),
      });
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900/70 border border-slate-800 rounded-3xl p-8 shadow-2xl">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-400 mt-2">Choose your application role.</p>
        </div>

        <div className="grid grid-cols-2 gap-2 mb-5">
          {(["CLAIMANT", "ADJUSTER"] as Role[]).map((item) => (
            <button
              key={item}
              type="button"
              onClick={() => setRole(item)}
              className={`rounded-xl py-3 text-sm font-semibold border ${role === item ? "border-cyan-500 bg-cyan-500/10 text-cyan-300" : "border-slate-700 text-slate-400"}`}
            >
              {item === "CLAIMANT" ? "Claimant" : "Adjuster"}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {role === "ADJUSTER" && (
            <input aria-label="Adjuster registration code" type="password" placeholder="Private adjuster registration code *" value={adjusterCode} onChange={(e) => setAdjusterCode(e.target.value)} className="input" />
          )}
          <div className="grid grid-cols-2 gap-3">
            <input aria-label="Password" type="password" placeholder="Password *" value={password} onChange={(e) => setPassword(e.target.value)} className="input" />
            <input aria-label="Confirm password" type="password" placeholder="Confirm *" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="input" />
          </div>
          <button type="submit" disabled={loading} className="w-full py-3.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 font-bold disabled:opacity-50">
            {loading ? "Creating..." : `Create ${role === "ADJUSTER" ? "Adjuster" : "Claimant"} Account`}
          </button>
        </form>
